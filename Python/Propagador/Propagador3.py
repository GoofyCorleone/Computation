...
# ============================================================================
# 3. VISUALIZACIÓN COMPLETA (SIN DIFERENCIA ABSOLUTA)
# ============================================================================
...
def visualizar_comparacion_completa(dif: DifraccionRectangular2DCompleta, 
    ...
    SIN gráfico de diferencia absoluta.
    ...
    # Crear figura con disposición 2x3
    ...
    bars = ax5.bar(metodos, valores, color=colores, alpha=0.7, width=0.6)
    ax5.set_ylabel('Intensidad en centro (0,0)')
    ax5.set_title('Comparación en centro')
    ...
    info_text = (
        ...
        f'• Relación Fresnel/HF: {I_centro_fresnel/I_centro_hf:.4f}'
    )
...
# ============================================================================
# 5. VISUALIZACIÓN DE PATRONES MÚLTIPLES (COMO FIGURA 11)
# ============================================================================

def visualizacion_comparativa_nf():
    """Visualización comparativa similar a la Figura 11 del documento."""
    
    print("VISUALIZACIÓN COMPARATIVA DE PATRONES DE DIFRACCIÓN")
    print("="*60)
    
    # Parámetros
    lambda_ = 500e-9  # 500 nm
    ancho_x = 1.0e-3  # 1 mm
    ancho_y = 1.0e-3  # 1 mm (cuadrado)
    
    # Números de Fresnel (similares a la figura)
    numeros_fresnel = [0.1, 0.5, 1.0, 2.0, 5.0, 10.0]
    
    # Crear figura grande
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))
    fig.suptitle('Comportamiento del patrón de difracción cuando varía el número de Fresnel\n'
                 f'λ = {lambda_*1e9:.0f} nm, Abertura: {ancho_x*1000:.1f} mm × {ancho_y*1000:.1f} mm',
    ...
    for idx, NF in enumerate(numeros_fresnel):
        ...
        factor = 2.0 + 10.0/NF if NF > 0.1 else 30.0
        rango = 5 * factor
        
        # Crear malla
        x_vals = np.linspace(-rango*1e-3, rango*1e-3, 100)
        y_vals = np.linspace(-rango*1e-3, rango*1e-3, 100)
        
        # Calcular mapa (solo Fresnel para velocidad)
        I_fresnel = dif.calcular_mapa_fresnel_2d(x_vals, y_vals)
        I_fresnel_norm = I_fresnel / np.max(I_fresnel)
        
        # Graficar
        ax = axes[idx]
        im = ax.imshow(I_fresnel_norm, 
                       extent=[-rango, rango, -rango, rango],
                       origin='lower', 
                       aspect='auto',
                       cmap='hot',
                       vmin=0, vmax=1)
        ...
        # Añadir barra de color al último gráfico
        if idx == len(axes) - 1:
            cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
            cbar.set_label('Intensidad (normalizada)')
    
    plt.tight_layout()
    plt.savefig("comparacion_NF.png", dpi=300, bbox_inches='tight')
    plt.show()
    
    print("\nFigura comparativa guardada como 'comparacion_NF.png'")

# ============================================================================
# 6. INTERFAZ INTERACTIVA MEJORADA
# ============================================================================
...
def interfaz_interactiva():
    ...
    while True:
        ...
        print("3. Usar valores por defecto (NF=1.0)")
        
        modo = input("Seleccione (1, 2 o 3): ").strip()
        ...
        visualizar_comparacion_completa(
            ...
            guardar_imagen=True,
            nombre_archivo=f"simulacion_interactiva_NF{dif.NF:.2f}.png"
        )
        
        # Preguntar si continuar
        print("\n" + "="*40)
        continuar = input("¿Realizar otra simulación? (s/n): ").strip().lower()
        if continuar != 's':
            print("¡Hasta luego!")
            break
...
def menu_principal():
    ...
    while True:
        ...
        print("3. Visualización comparativa (como Figura 11)")
        print("4. Interfaz interactiva (personalizar parámetros)")
        print("5. Salir")
        ...
        if opcion == '1':
            print("\n" + "="*40)
            print("SIMULACIÓN CON PARÁMETROS POR DEFECTO")
            print("="*40)
            ...
            print("\n" + "="*40)
            print("VISUALIZACIÓN COMPARATIVA (COMO FIGURA 11)")
            print("="*40)
            print("Comparación de patrones para diferentes NF")
            print("λ = 500 nm, Abertura cuadrada 1 mm × 1 mm")
            
            visualizacion_comparativa_nf()
