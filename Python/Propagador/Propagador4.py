    ...
    def perfil_x_fresnel(self, x_vals: np.ndarray, y_fijo: float = 0.0, 
        ...
        for i, x in enumerate(x_vals):
            ...
            def integral_en_x(xp):
                if abs(xp) <= self.ancho_x/2:
                    ...
                    if registrar_puntos and abs(x) < 1e-10:  # Solo para x=0
                        ...
                        int_y, info = integrador.simpson(
                            lambda yp: integral_en_y(yp),
                            -self.ancho_y/2, self.ancho_y/2,
                            tol=1e-8
                        )
                        self.info_integracion['y'] = info
                    else:
            ...
            if registrar_puntos and abs(x) < 1e-10:  # Solo para x=0
                ...
                self.info_integracion['x'] = info
                info_integracion = self.info_integracion
            else:
...
# ============================================================================
# 3. VISUALIZACIÓN COMPLETA CON MALLA DE INTEGRACIÓN
# ============================================================================

def visualizar_con_malla_integracion(dif: DifraccionRectangular2DCompleta, 
                                     x_range_mm: Tuple[float, float] = (-5, 5),
                                     y_range_mm: Tuple[float, float] = (-5, 5),
                                     resolucion_2d: int = 100,
                                     mostrar_malla: bool = True,
                                     guardar_imagen: bool = False,
                                     nombre_archivo: str = None):
    """
    Visualización completa con mapas 2D, perfiles 1D y malla de integración.
    """
    ...
    # Calcular perfiles 1D con registro de puntos para la malla
    print("\nCalculando perfiles 1D...")
    inicio_perfiles = time.time()
    
    # Perfil en X (y=0) - registrar puntos para malla
    perfil_x_f, info_malla = dif.perfil_x_fresnel(x_vals, y_fijo=0.0, registrar_puntos=mostrar_malla)
    ...
    if mostrar_malla and info_malla and 'x' in info_malla:
        # Extraer puntos de la malla de integración en x
        puntos_x = np.array([p[0] for p in info_malla['x']['puntos_evaluados']])
        valores_x = np.array([p[1] for p in info_malla['x']['puntos_evaluados']])
        
        # Graficar la malla
        ax5.scatter(puntos_x * 1000, np.abs(valores_x), 
                   color='green', alpha=0.6, s=30, label='Puntos de evaluación')
        
        # Conectar puntos con líneas para mostrar la malla
        orden = np.argsort(puntos_x)
        ax5.plot(puntos_x[orden] * 1000, np.abs(valores_x[orden]), 
                'g-', alpha=0.3, linewidth=1)
        
        # Marcar el intervalo de integración
        ax5.axvspan(-dif.ancho_x/2 * 1000, dif.ancho_x/2 * 1000, 
                   alpha=0.1, color='blue', label='Abertura')
        
        ax5.set_xlabel('x\' (mm)')
        ax5.set_ylabel('|Integrando|')
        ax5.set_title(f'Malla de Integración (Simpson Adaptativo)\n'
                     f'{info_malla["x"]["num_puntos"]} puntos, '
                     f'{info_malla["x"]["llamadas_funcion"]} evaluaciones')
        ax5.legend(loc='upper right', fontsize=9)
        ax5.grid(True, alpha=0.3)
        
        # Información adicional sobre la integración
        info_text = (f'Profundidad máxima: {info_malla["x"]["profundidad_maxima"]}\n'
                    f'Puntos únicos: {info_malla["x"]["num_puntos"]}\n'
                    f'Evaluaciones totales: {info_malla["x"]["llamadas_funcion"]}')
        
        ax5.text(0.02, 0.98, info_text, transform=ax5.transAxes,
                verticalalignment='top', fontsize=8,
                bbox=dict(boxstyle='round', facecolor='lightgray', alpha=0.8))
    else:
        ax5.text(0.5, 0.5, 'Malla no disponible\n(mostrar_malla=False o sin datos)',
                horizontalalignment='center', verticalalignment='center',
                transform=ax5.transAxes, fontsize=12)
        ax5.set_title('Malla de Integración')
        ax5.axis('off')
    ...
    info_text = (
        f'INFORMACIÓN:\n\n'
        f'• Número de Fresnel (NF): {dif.NF:.4f}\n'
        f'• Distancia z: {dif.z*100:.2f} cm\n'
        f'• Longitud de onda λ: {dif.lambda_*1e9:.1f} nm\n'
        f'• Ancho X: {dif.ancho_x*1000:.3f} mm\n'
        f'• Ancho Y: {dif.ancho_y*1000:.3f} mm\n'
        f'• Número de onda k: {dif.k:.2e} m⁻¹\n\n'
        f'RESULTADOS:\n\n'
        f'• Intensidad máxima Fresnel: {np.max(I_fresnel_2d):.4e}\n'
        f'• Intensidad máxima Huygens-Fresnel: {np.max(I_hf_2d):.4e}\n'
        f'• Diferencia en perfil X: {diff_x:.4f}\n'
        f'• Diferencia en perfil Y: {diff_y:.4f}'
    )
    ...
    if guardar_imagen:
        if nombre_archivo is None:
            nombre_archivo = f"difraccion_con_malla_NF{dif.NF:.2f}.png"
    ...
    # Información adicional de la malla si está disponible
    if mostrar_malla and info_malla and 'x' in info_malla:
        print(f"\nINFORMACIÓN DE INTEGRACIÓN:")
        print(f"  Puntos de evaluación en x: {info_malla['x']['num_puntos']}")
        print(f"  Evaluaciones totales en x: {info_malla['x']['llamadas_funcion']}")
        print(f"  Profundidad máxima recursión: {info_malla['x']['profundidad_maxima']}")

# ============================================================================
# 4. FUNCIONES PARA VISUALIZAR SOLO LA MALLA DE INTEGRACIÓN
# ============================================================================

def visualizar_malla_detallada(dif: DifraccionRectangular2DCompleta, 
    ...
    print(f"\nVisualizando malla de integración para (x={x_observacion*1000:.1f} mm, y={y_observacion*1000:.1f} mm)")
...
def menu_principal():
    """Menú principal del programa con opción de malla de integración."""
    
    print("DIFRACCIÓN RECTANGULAR 2D - FRESNEL vs HUYGENS-FRESNEL")
    print("="*60)
    print("Simulaciones numéricas con método de Simpson auto-adaptativo")
    print("="*60)
    ...
    while True:
        ...
        print("="*50)
        print("1. Simulación completa con malla de integración (NF=1.0)")
        print("2. Simulación completa SIN malla de integración (NF=1.0)")
        print("3. Visualizar solo malla de integración (detallada)")
        print("4. Simulación para varios números de Fresnel")
        ...
        if opcion == '1':
            print("\n" + "="*50)
            print("SIMULACIÓN COMPLETA CON MALLA DE INTEGRACIÓN")
            print("="*50)
            ...
            visualizar_con_malla_integracion(
                dif,
                x_range_mm=(-5, 5),
                y_range_mm=(-5, 5),
                resolucion_2d=80,
                mostrar_malla=True,
                guardar_imagen=True,
                nombre_archivo=f"completa_con_malla_NF{dif.NF:.2f}.png"
            )
        
        elif opcion == '2':
            print("\n" + "="*50)
            print("SIMULACIÓN COMPLETA SIN MALLA DE INTEGRACIÓN")
            print("="*50)
            ...
            visualizar_con_malla_integracion(
                ...
                mostrar_malla=False,
                guardar_imagen=True,
                nombre_archivo=f"completa_sin_malla_NF{dif.NF:.2f}.png"
            )
            ...
            visualizar_malla_detallada(dif, x_observacion=x_obs, y_observacion=y_obs)
            ...
            for NF in numeros_fresnel:
                ...
                visualizar_con_malla_integracion(
                    ...
                    resolucion_2d=70,
                    mostrar_malla=False,  # Sin malla para velocidad
                    guardar_imagen=True,
                    nombre_archivo=f"varios_NF/rectangular_NF{NF:.1f}.png"
                )
...
def interfaz_interactiva():
    ...
    while True:
        ...
        # ¿Mostrar malla?
        mostrar_malla = input("¿Mostrar malla de integración? (s/n, por defecto s): ").strip().lower()
        mostrar_malla = mostrar_malla != 'n'
        ...
        print(f"  Mostrar malla = {'Sí' if mostrar_malla else 'No'}")
