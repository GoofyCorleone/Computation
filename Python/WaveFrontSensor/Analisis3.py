...
plt.rcParams['figure.figsize'] = [16, 12]  # Figura más grande
...
def analizar_aberracciones_shack_hartmann(archivo_csv):
    ...
    if 'Z_medido' in locals():
        ...
        ax1.set_title('Mapa del Frente de Onda (Datos no disponibles)', fontsize=14, pad=15)
    ...
    if 'Z_medido' in locals() and 'X' in locals() and 'Y' in locals():
        ...
        ax2.set_title('Frente de Onda 3D (Datos no disponibles)', fontsize=14, pad=15)
        ...
        for i, (bar, coef) in enumerate(zip(barras, coeficientes)):
            if abs(coef) > 3.0:
                ...
                ax3.text(bar.get_x() + bar.get_width()/2., height,
                        f'Z{i}', ha='center', va='bottom', fontsize=9)
    else:
        ax3.text(0.5, 0.5, 'No hay coeficientes', ha='center', va='center', 
    ...
    if coeficientes and len(coeficientes) <= len(nombres_aberracciones):
        ...
        if aberraciones_significativas:
            ...
            bars = ax4.barh(aberraciones_significativas, valores_significativos, 
                           color=colores, alpha=0.7, height=0.6)  # Barras más delgadas
            ...
            plt.setp(ax4.get_yticklabels(), fontsize=9)  # Tamaño de fuente más pequeño
    ...
    if 'Z_medido' in locals() and 'X' in locals() and 'Y' in locals():
        r = np.sqrt(X**2 + Y**2)
        ax5.scatter(r.flatten(), Z_medido.flatten(), alpha=0.5, s=2)  # Puntos más pequeños
        ...
        ax5.set_title('Distribución Radial (Datos no disponibles)', fontsize=14, pad=15)
